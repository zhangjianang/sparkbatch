...
public class Source {
    ...
    public static void main(String[] args) {
        ...
        //mainRDD = mainRDD.persist(StorageLevel.DISK_ONLY_2());
        ...
        //annualRDD = annualRDD.persist(StorageLevel.DISK_ONLY_2());

        //转换annual
        JavaPairRDD<String, Iterable<String>> newAnnualRDD = convertAnnualRDD(annualRDD);
        //newAnnualRDD = newAnnualRDD.persist(StorageLevel.DISK_ONLY_2());


        //mainRDD和newMortgageRDD连接
        JavaPairRDD<String, Tuple2<String, Optional<Iterable<String>>>> join = converMainRDD.leftOuterJoin(newAnnualRDD, 200);
        //join = join.persist(StorageLevel.DISK_ONLY_2());
        ...
        //mainAndAnnualRDD = mainAndAnnualRDD.persist(StorageLevel.DISK_ONLY_2());
        ...
        //
        JavaPairRDD<String, Tuple2<String, Optional<Iterable<String>>>> join2 = mainAndAnnualRDD.leftOuterJoin(newMortgageRDD,200);
        //join2 = join2.persist(StorageLevel.DISK_ONLY_2());


        //最终RDD
        JavaRDD<ResultModel> finalRDD = convertFinalModelRDD(join2);

        /*finalRDD.foreachPartition(new VoidFunction<Iterator<ResultModel>>() {
            @Override
            public void call(Iterator<ResultModel> v1) throws Exception {
                Connection c = SQLUtil.getConn("com.mysql.jdbc.Driver","jdbc:mysql://127.0.0.1:3306","test","root","wang863452");
                String sql = "insert into companyinfo (name,json) values(?,?)";
                PreparedStatement s = c.prepareStatement(sql);
                while(v1.hasNext()){
                    ResultModel next = v1.next();
                    s.setString(1,next.getName());
                    s.setString(2,next.getJson());
                    s.addBatch();
                }
                s.executeBatch();

                SQLUtil.colseCon(c,s,null);
        ...
        Dataset<Row> dataFrame = spark.createDataFrame(finalRDD, ResultModel.class);
       ...
       /* final static String url = "jdbc:mysql://192.168.0.197:3306/prism1";
        ...
        /*dataFrame.write()
                ...
                .option("dbtable", "test.companyinfo")
                .option("user", "root")
                .option("password", "wang863452")
        ...
        dataFrame.write()
                .format("jdbc")
                .option("url", "jdbc:mysql://prod-iac-info.cpjun6wiviel.rds.cn-north-1.amazonaws.com.cn:3306")
                .option("dbtable", "commercial.companyinfo")
                .option("user", "finder")
                .option("driver","com.mysql.jdbc.Driver")
                .option("password", "xsycommercial123")
                .mode(SaveMode.Overwrite)
                .save();
    ...
    private static JavaPairRDD<String,Iterable<String>> convertMortgageRDD(JavaPairRDD<String, Iterable<String>> mortRDD) {
        ...
        }).groupByKey(200).filter(new Function<Tuple2<String, Iterable<String>>, Boolean>() {
            @Override
...
*/
    ...
    private static JavaPairRDD<String,Iterable<String>> createMainRDD(SparkSession spark) {
        ...
        Dataset<Row> load =spark.sql("select" +
                ...
                "parent_id" +
                " from company  where company_org_type!='个体户' ");/*where company_org_type!='个体户'*/
